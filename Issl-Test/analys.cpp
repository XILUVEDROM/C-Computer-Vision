...
#include "obrabotkaalg.h"

//QVector<QVector<quint64>>

const QRgb black = qRgb(0,0,0);

inline bool inner(qint32 x, qint32 y,
                  QVector<QVector<quint64>>& V)
{//return true if d is inner point
    //dangerous function. May cause out of range!
    return (V[y][x + 1])&&(V[y][x - 1])
            &&(V[y + 1][x])&&(V[y - 1][x]);
}

inline bool isBlack(qint32 x, qint32 y,
                    const QImage& im)
{
    return im.pixel(x,y) == black;
}

void fill(const QImage& img,
          QVector<QVector<quint64>>& V,
          qint32 _x, qint32 _y, quint64 L)
{
    QPoint t;
    QStack<QPoint> depth;
...
void Analys::select()//heavy function
{//main calculate
    qint32 const _h = bin.height();
    qint32 const _w = bin.width();
    quint64 L = 1; // starting id value

    QVector<QVector<quint64>> Labels
            (_h, QVector<quint64>(_w,0));
    ...
    QVector<QVector<quint64>> data_01
            = std::move(Labels);
    QVector<Obrabotka> data_obj = std::move(V);
